...
session_start();
include('connect.php');
$selectdest="select * from packages inner join destinations on destination_id=dest_id and id=".$_SESSION['package_id']."";
$resultdest=$conn->query($selectdest);

while($rowdest=mysqli_fetch_assoc($resultdest)){
//   $_SESSION['destinationid']=$rowdest['blog_id'];
...
<head>
    <meta charset="utf-8">
    <title>Pulmmer Tours | Details</title>
            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="book_button">
                        <span class="package_id"hidden><?php echo $_SESSION['package_id'];?></span>
                   <button class="bg-primary border-primary text-white"style="border:0;margin-bottom:20px;width:50%;"id="btn-book">BOOK</button>
                </div>
                    <div class="row pb-3"id="blog_panel">
                        
                        <!-- php -->
                        <?php
                        $destinationid=$_SESSION['package_id'];
                        $selectCat="select * from package_images where package_id='$destinationid'";
                    $result=$conn->query($selectCat);
                    while($row=mysqli_fetch_assoc($result)){
                        ?>
                       <div id="carouselExampleControls" class="carousel slide blog_carousel" data-ride="carousel"style="width:100%;">
                        ...
                        <?php
                            ...
                            <img class="d-block w-100" src="<?= 'uploads/package/'.$rows['pack_image'];?>"alt="First slide"style="width:100%">
                            </div>
                            <?php
                            $i++;
                                }
                        ...
                        <div class="blog_body"style="margin-top:20px">
                        <h4 class="text-uppercase mb-4"><span style="color:rgb(0,130,189">TO:</span> <?php echo $rowdest['destination_name'];?></h4>
                        <h4 class="text-uppercase mb-4"><?php echo $rowdest['package_name'];?></h4>
                        <?php echo $rowdest['activities'];?>
                        </div>
...
<script>
    $(document).ready(function(){
        $('#btn-book').on('click',function(){
            var id= $(this).closest('.book_button').find('.package_id')[0].innerText
            $.ajax({
                url:'book.php',
                method:'POST',
                data:{id:id},
